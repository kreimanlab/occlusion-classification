...
for category = 1:numel(categories)
    ...
    humanHumanCorrs(category) = corr(...
        reshape(humanCorrectHalfs(category, :, 1), [presPerCategory 1]), ...
        reshape(humanCorrectHalfs(category, :, 2), [presPerCategory 1]));
end
    ...
    for model = 1:numel(corrData.modelNames)
        for timeIter = 1:numTimes
            ...
            modelHumanCorr(category, model, timeIter) = corr(...
                reshape(modelCorrect(category, model, timeIter, :), [presPerCategory 1]), ...
                reshape(humanCorrect(category, :), [presPerCategory 1]));
        ...
        plot(categoryXLeft:categoryXRight, ...
            reshape(modelHumanCorr(category, model, :), [numTimes 1]), ...
